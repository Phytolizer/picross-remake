...
//			private int[][] matrixStats; // used to generate statistics of which matrix cells were popular
		...
		public void updateSolver(byte[] line, byte[] blocks, int blockSums[], boolean isRow, byte[][] initSol, boolean[][] initMem, int maxPos){
			...
			solvedLine = (byte[])this.line.clone();
		...
		public byte[] solveLine(){
			...
			this.hasSolution = sol[getMatIndex(line.length-1,blocks.length)] == true;
			if (!this.hasSolution)
		 ...
		 */
		private final boolean canPlaceBlock(int position, int length) {
			if (position < 0)
				return false;
...
//						System.out.println("setting cell "+position+" as both colors");
		 ...
		 */
		private final boolean getSol(int position, int job){
			
			if (position == -1)  
				if (job == 0) // finished placing the last block, exacly at the beginning of the line.
					return true;
				else
					return false;
		...
		*/
		...
		public static boolean[][] cloneBoolMat(boolean[][] original){
			...
			for (int i=0; i < original.length; i++)
				ans[i] = (boolean[])original[i].clone();
			return ans;
		}
