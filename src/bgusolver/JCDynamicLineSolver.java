...
//	
    ...
    JCDynamicLineSolver(byte boardContents[][], byte rowBlocks[][], byte columnBlocks[][], int numRows, int numCols, int animateDelay, JCSolveSingleLineTOPDOWN ssl) {
        ...
        linesToSolve = new PriorityQueue<>(100, new JCLineRequestComperator());
        ...
        if (boardContents != null) {
            this.boardContents = boardContents.clone();
        }
        this.ssl = ssl;
    ...
    public boolean solveCrosswordByLines(boolean initWithCell, int row, int col) {
        ...
        if (initWithCell) {
            ...
            // Adding all rows
            linesToSolve.addAll(Arrays.asList(rowLineRequests).subList(0, numRows));

            // Adding all cols
            linesToSolve.addAll(Arrays.asList(colLineRequests).subList(0, numCols));
        }
    ...
    private boolean solveLine(boolean isRow, int selectedLine) {
        ...
        if (hashValue != null) {
            ...
            hashTable.put(hashKey, new JCLineHashValue(solvedLine.clone(), true, ssl.numCellsSolved));
            return true;
...
//		}
     ...
     */
    private String lineToString(byte[] line, byte[] blocks) {
    ...
    private void printLineStats(byte[] line, byte[] blocks, boolean newInfo) {
